...
#putting ys in terms of xs for aggregate
agg_step1<- (1/SectorA_Abatement+1/SectorB_Abatement+1/SectorC_Abatement)

##step two of aggregate to get new intercept of equation
...
#plug (300) into agg abatement to get price of tax:# 39.43
...
intersectA<-curve_intersect(price_carbon,SA_CA, empirical = FALSE, domain=c(0,300)) 
tax_costA = integrate(SA_CA, 0, intersectA$x) #1282.612

# Find the intersection of B curve & price of carbon

intersectB<-curve_intersect(price_carbon,SB_CA, empirical = FALSE, domain=c(0,300))

tax_costB = integrate(SB_CA, 0, intersectB$x) #3692.271


# Find the intersection of C curve & price of carbon
intersectC<-curve_intersect(price_carbon,SC_CA, empirical = FALSE, domain=c(0,300)) 

tax_costC = integrate(SC_CA, 0, intersectC$x)
#904.9294
...
##why is the tax revenue and the total tax of three sectors different? shouldnt they be the same?





##GRAPH FOR AGG DEMAND

graph_agg <- ggplot(abate) +
  marginal_graph <- ggplot(abate) +
  geom_smooth(aes(Abatement,MC_A, color="Sector A"), method=lm,se=FALSE) +
  geom_smooth(aes(Abatement,MC_B,color=" Sector B"), method=lm, se=FALSE) +
  geom_smooth(aes(Abatement,MC_C, color="Sector C"), method=lm, se=FALSE) +
  geom_smooth(aes(Abatement,MC_D, color="Sector D"), method=lm, se=FALSE) +
  stat_function(fun=agg_abatement, color="black") +
  stat_function(fun=price_carbon, color="brown") +
  scale_x_continuous(expand = c(0,0), breaks=seq(0,500, by=50)) +
  theme_classic()
